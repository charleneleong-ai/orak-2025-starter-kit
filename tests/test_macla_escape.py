...
# ── use_procedure_layer master switch (Stage B' baseline) ──────────────


def test_localconfig_use_procedure_layer_defaults_to_true():
    """Default preserves the current Stage D behavior — procedure cache ON."""
    from config.agent_config import LocalConfig
...
def test_select_procedure_short_circuits_when_layer_disabled():
    ...
    ``(None, 0.0)`` without consulting EU / theta / streaks. Used by the
    Stage B' baseline to isolate the "raw LLM with reflection aids" path."""
    from agents.macla.macla_lib import (
        BayesianProcedureSelector,
        EnhancedHierarchicalMemorySystem,
        ProceduralMemoryEntry,
    ...
    mem = EnhancedHierarchicalMemorySystem()
    high_eu = ProceduralMemoryEntry(procedure=_mk_procedure(["a"]))
    mem.procedural_memory["proc_high"] = high_eu

    selector = BayesianProcedureSelector(memory_system=mem)
    selector.use_procedure_layer = False
    # Stub a candidate with very high EU — even so, the master switch wins
...
def test_macla_agent_forwards_use_procedure_layer_to_selector():
    """EnhancedMACLAAgent threads the flag through to the selector instance."""
    from agents.macla.macla_lib import EnhancedMACLAAgent

    agent = EnhancedMACLAAgent(use_procedure_layer=False)
    assert agent.bayesian_selector.use_procedure_layer is False
