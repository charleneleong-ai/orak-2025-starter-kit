...
from agents._harness import ErrorClass
...
def test_trajectory_writer_recent_returns_last_k(tmp_path: Path):
    w = TrajectoryWriter(tmp_path)
    for i in range(5):
        w.add_step(StepRecord(step=i, system_prompt="s", user_prompt=f"u{i}",
...
def test_format_recent_history_action_truncation():
    ...
    records = [
        StepRecord(step=1, system_prompt=None, user_prompt="u", assistant_output="a",
                   action=long_action, info_score=1.0, obs_digest="x"),
    ]
    out = format_recent_history(records)
...
def test_step_record_to_sharegpt_with_outcome_fields():
    r = StepRecord(
        step=4, system_prompt="sys", user_prompt="u",
        assistant_output="a", action="left",
        info_score=2.0, obs_digest="deadbeef",
    )
    sg = r.to_sharegpt()
    assert sg["step"] == 4
