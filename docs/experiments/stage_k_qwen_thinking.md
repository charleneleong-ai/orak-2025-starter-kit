# Stage K — Qwen 3 Thinking-Mode (Extended Reasoning at Decision Time)

**Status:** scaffolded, queued after Stage H iter 3  •  **Branch:** `feat/stage-k-qwen-thinking`  •  **Sequenced before:** Stage J
...
Stage K tests one specific intervention: **explicit thinking-mode reasoning budget before each tool-call**.
...
| ~ 57.14% (within σ ≤ 7pp) | **Thinking doesn't lift.** Either same plateau for same reason (state-observation limit, action-vocabulary limit) OR gain from wrong reasoning style. **Stage J cumulative memory becomes the next test.** |
...
## Why this is the *right* next test (not Stage J)

- **Stage H is the direct precursor**: Stage K is one variable change. Cleanest experiment.
...
- **Result informs Stage J priority**: if thinking lifts past 57.14% → cumulative memory might compound the gain. If thinking doesn't lift → Stage J tests a totally different lever (state-carryover, not reasoning-depth).
...
# Launch Stage K n=3
nohup bash experiments/stage_k_qwen_thinking/run_pokemon_n3.sh \
...
- **Stage J cumulative memory** — queued AFTER Stage K. If Stage K lifts, Stage J becomes "does cumulative memory + thinking-mode compound?". If Stage K doesn't lift, Stage J becomes the standalone test of a different lever.
