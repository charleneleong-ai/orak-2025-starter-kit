...
| pokemon | PR #31 planner-prompt re-run pending | **Baseline first** | Need to know if Stage D-with-waypoint-prompt beats Stage A before training |
...
PR #28 cross-game ablation (n=2): Stage A=35.18, Stage C=35.18 (+0%, vmem null),
**Stage D=43.90 (+25%, planner helps)**. Bottleneck was MACLA procedure compounding
via carry-over (PR #22), not model quality. Config locked to Stage D — `use_vector_memory: false`
would match the 43.90 ceiling at lower inference cost since vmem gives no lift here, but
the existing Stage D config bundles them. Training mario trajectories risks overfitting
to W1-1 layout.

### 2048 — GRPO target

PR #28 cross-game ablation (n=2): Stage A=15.91, **Stage C=23.50 (+48%, vmem helps)**,
Stage D=18.09 (−23% vs C, planner hurts). Config locked to Stage C: `use_vector_memory: true`,
`use_subtask_planning: false`. Bottleneck: combinatorial state space means SFT from human
trajectories has limited ceiling. The corner-anchor strategy needs consistent discovery,
not just occasional imitation.
...
PR #28 cross-game ablation (n=2 every cell, May 2026):
- Stage A: **14.29%** (escapes RedsHouse → reaches PalletTown → BluesHouse)
- Stage C (vmem): **14.29%** — vmem null on pokemon (no lift over A)
- Stage D (vmem + planner): **0.00%** — planner traps agent in starter house, never escapes
  to PalletTown; the generic `agents/_cognitive/subtask_planner.py` generates abstract
  subgoals like "explore upstairs" / "check items" from pokemon's `DEFAULT_GOAL =
  "Become Pokémon Champion"` and overrides the agent's prompt-level exit-vs-staircase hint.

Architectural fix in flight: [PR #31](https://github.com/charleneleong-ai/orak-2025-starter-kit/pull/31) adds a per-adapter `SUBTASK_PLANNER_SYSTEM` override and a pokemon-specific system prompt embedding the early-game waypoint chain (`RedsHouse → PalletTown → OaksLab → starter → rival → Route 1`). Validation: Stage D pokemon re-run on this branch — pass criterion ≥ 14.29.

Action sequence (gated on the validation re-run):
1. **Re-run Stage D pokemon on PR #31's branch** — fills the post-fix Stage D-with-waypoint cell
2. **If Stage D-with-waypoint > Stage A**: pokemon ships Stage D; train DPO with map-discovery episodes as `chosen`
3. **If Stage D-with-waypoint ≈ Stage A**: pokemon ships Stage A as fallback; train KTO with "reached PalletTown" as desirable
4. **GRPO** only after the per-game baseline lands and the reward signal is dense post-fix
...
to discover the path via exploration from a cold start. This complements
PR #31's prompt-level waypoint chain — the same milestones expressed in
...
| PR #31 Stage D pokemon validation re-run | 0 LOC, 2 episodes | PR #31 merged | pokemon DPO/KTO (need to know which stage to train on) |
