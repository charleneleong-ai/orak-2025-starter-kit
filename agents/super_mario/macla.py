...
class MarioMaclaAgent(BaseMaclaAgent, SuperMarioAgent):
    ...
    def _get_task_description(self, task_description: str | dict[str, str] = None) -> str:
        if isinstance(task_description, dict):
            return task_description.get("task_description", "Complete stage 1-1 by moving right and avoiding enemies.")
    ...
    def _extract_context(self, observation: str) -> str:
        ...
        if mario_match:
            ...
            if x_match: mario_x = int(x_match.group(1))
            if y_match: mario_y = int(y_match.group(1))
        ...
        entity = extract_entity_with_relative_position(["monster goomba", "goomba", "enemy"], "goomba")
        if entity: 
            logger.debug(f"Extracted goomba: {entity}")
            context_parts.extend(entity)
        
        entity = extract_entity_with_relative_position(["monster koopas", "koopa", "turtle"], "koopa")
        if entity: 
    ...
    def _detect_success(self, execution_result: dict, prev_state_str: str, cur_state_str: str) -> tuple[bool, bool]:
        ...
        try:
            ...
            if p_match: prev_x = int(p_match.group(1))
            if c_match: cur_x = int(c_match.group(1))
    ...
    def extract_postconditions(self, success_contexts):
        ...
        if start_xs and end_xs:
            ...
            if avg_dist > 5:
                ...
                changes["postconditions_added"].add(f"moved_forward_approx_{int(avg_dist/10)*10}")
    ...
    @weave.op()
    def _get_action(self, task_description: str, cur_state_str: str, obs_image: Any = None) -> tuple[str, str, str, Any, str, str, str, dict]:
