...
class PoetiqTwentyFourtyEightAgent(TwentyFourtyEightAgent):
    ...
    _failure_mode: str | None = PrivateAttr(default=None)
    ...
    def _format_history_for_prompt(self) -> str:
        ...
        output = []
        for i, (score, max_tile, instructions, _failure_mode, _steps) in enumerate(
            sorted_strategies, 1
        ):
            output.append(
    ...
    def _evolve_instructions(
        ...
        logger.info(
            f"Evolving instructions... (Initial: {initial}, Last Score: {last_score}, Max Tile: {last_max_tile})"
        )
    ...
    def get_action(self, obs: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        system_prompt = PLAYER_SYSTEM_PROMPT_TEMPLATE.format(
    ...
    def record_episode_end(self, episode: int, game_name: str, seed: str, score: int):
        ...
        heapq.heappush(
            ...
            (score, self._last_max_tile, self._current_instructions, self._failure_mode, steps),
        )
