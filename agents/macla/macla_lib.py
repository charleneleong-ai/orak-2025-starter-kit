...
class EnhancedHierarchicalMemorySystem:
    ...
    @weave.op()
    def add_atomic_entry(self, action: str, observation: str, reward: float, context: str, trajectory_id: str = "", step_index: int = 0, goal: str = "unknown"):
    ...
    @weave.op()
    def add_procedural_entry(self, procedure: Procedure, contexts: Set[str], goals: Set[str], performance: float) -> str:
    ...
    @weave.op()
    def record_execution_outcome(self, proc_key: str, success: bool, context: ContrastiveContext, is_fatal: bool = False):
...
class BayesianProcedureSelector:
    def __init__(self, memory_system: EnhancedHierarchicalMemorySystem, context_extractor=None, spatial_pattern_extractor=None):
    ...
    def _compute_failure_risk(self, entry: ProceduralMemoryEntry, observation: str, theta_risk: float = 0.85) -> float:
    ...
    def _compute_expected_utility(self, entry: ProceduralMemoryEntry, observation: str, goal: str) -> float:
        ...
        risk = self._compute_failure_risk(entry, observation, theta_risk=getattr(self, '_adaptive_theta_risk', 0.85))
        info_gain = self._compute_information_gain(entry.procedure)

        eu = (relevance * rho_mean * 1.0) - (risk * (1 - rho_mean) * 0.5) + 0.1 * info_gain
        return max(0.0, eu)

    def select_procedure(self, observation: str, goal: str, theta_conf: float = 0.25) -> tuple[Optional[str], float]:
        ...
        logger.debug(f"[Selector] best_eu={best_eu:.3f} theta={theta_conf:.3f} candidates={len(utilities)} pk={best_pk}")
...
class ContrastiveRefinementEngine:
    def __init__(self, n_min_s: int = 3, n_min_f: int = 2, postcondition_extractor=None, spatial_pattern_extractor=None):
    ...
    def should_refine(self, entry: ProceduralMemoryEntry) -> bool:
        ...
        has_enough_data = len(entry.success_contexts) >= self.n_min_s and len(entry.failure_contexts) >= self.n_min_f
    ...
    def refine_procedure(self, entry: ProceduralMemoryEntry) -> ProceduralMemoryEntry:
        ...
        if new_preconditions:
            ...
            logger.info(f"Refined procedure {entry.procedure.goal}: added discriminative preconditions {new_preconditions}")
...
class MetaProceduralLearner:
    def __init__(self, memory_system: EnhancedHierarchicalMemorySystem, precondition_extractor=None):
        self.memory_system = memory_system
        ...
        if start_context:
            if self.precondition_extractor:
                ...
                if ',' in start_context:
                    ...
                    i = 0
                    while i < len(parts):
                        if i + 2 < len(parts) and parts[i+1].startswith('x') and parts[i+2].startswith('y'):
    ...
    def _generate(self, prompt: str) -> str:
        try:
            if callable(self.generator):
                ...
                logger.warning("FrozenLLMReasoner: generator is neither callable nor has invoke method.")
                return ""
        except Exception as e:
...
class EnhancedMACLAAgent:
    def __init__(self, N_a: int = 1000, N_s: int = 100, N_p: int = 200, N_m: int = 50, 
                 context_extractor=None, postcondition_extractor=None, spatial_pattern_extractor=None, precondition_extractor=None, refinement_config: dict = None):
        self.memory_system = EnhancedHierarchicalMemorySystem(N_a, N_s, N_p, N_m)
        self.bayesian_selector = BayesianProcedureSelector(self.memory_system, context_extractor=context_extractor)
        self.precondition_extractor = precondition_extractor
        ...
        self.meta_learner = MetaProceduralLearner(self.memory_system, precondition_extractor=precondition_extractor)
    ...
    def _compute_adaptive_theta(self) -> float:
        ...
        logger.debug(f"[Adaptive] theta={theta:.3f} fatal_rate={fatal_rate:.2f} total={total} stagnant={s['stagnant_episodes']}")
        return theta
    ...
    def provide_feedback(self, execution_result: dict, actual_success: bool, next_observation: str = "", next_obs_image: Any = None, is_fatal: bool = False, shaped_reward: float | None = None) -> dict:
        ...
        """
        logger.debug(f"provide_feedback: next_obs_image type={type(next_obs_image)}, is_none={next_obs_image is None}")
        logger.debug(f"provide_feedback: execution_result obs_image type={type(execution_result.get('obs_image'))}, is_none={execution_result.get('obs_image') is None}")
        ...
        if pk and pk in self.memory_system.procedural_memory:
            ...
            logger.debug(f"Recording outcome for existing procedure {pk}. {execution_result.get('obs_image')}, {next_obs_image}")
            ...
            if action_seq and goal and obs:
                ...
                logger.debug(f"Creating temporary ContrastiveContext for new procedure learning {execution_result.get('obs_image')}.")
    ...
    def learn_from_trajectories(self):
        ...
        try:
            ...
            for traj_id, entries in trajectories.items():
                ...
                if len(action_seq) >= 5 and total_reward > 0:
                    ...
                    task_name = f"{trajectory_goal}_survival" if trajectory_goal != "unknown" else "survival"
            ...
            if len(self.memory_system.atomic_memory) > 100 and (not self.bayesian_selector.ontology or len(trajectories) > 5):
    ...
    def get_detailed_memory_stats(self) -> dict:
        ...
        mem_stats = self.memory_system.stats
        
        # Calculate derived metrics
        total_procs = len(self.memory_system.procedural_memory)
        avg_success = 0.0
        if total_procs > 0:
            avg_success = sum(entry.procedure.success_rate for entry in self.memory_system.procedural_memory.values()) / total_procs
    ...
    def log_procedures(self) -> dict:
        ...
        for key, entry in self.memory_system.procedural_memory.items():
            ...
            logger.info(f"ID: {key} | Goal: {proc.goal} | Success Rate: {proc.success_rate:.2f} | Execs: {proc.execution_count}")
            ...
            logger.info(f"  Success contexts: {len(entry.success_contexts)}, Failure contexts: {len(entry.failure_contexts)}")
        ...
        for key, meta in self.memory_system.meta_procedural_memory.items():
            logger.info(f"ID: {key} | Goal: {meta.goal_meta} | Success Rate: {meta.success_rate:.2f}")
...
class LLMMACLAAgent(EnhancedMACLAAgent):
    def __init__(self, generator: Any, fallback_generator: Optional[Any] = None, N_a=1000, N_s=100, N_p=200, N_m=50, 
                 context_extractor=None, postcondition_extractor=None, spatial_pattern_extractor=None, precondition_extractor=None, refinement_config: dict = None):
        super().__init__(N_a, N_s, N_p, N_m, context_extractor=context_extractor, 
                         postcondition_extractor=postcondition_extractor, 
                         spatial_pattern_extractor=spatial_pattern_extractor,
    ...
    def _generate_fallback_actions(self, goal: str, observation: str, **kwargs) -> list[str] | tuple[list[str], str]:
        """
