...
class BaseMaclaAgent(BaseModel):
    ...
    _update_counters: dict = PrivateAttr(
        default_factory=lambda: {"atomic_entry": 0, "procedure_learned": 0, "procedure_updated": 0}
    )
    ...
    def _init_macla_agent(self):
        ...
        if isinstance(self.config, LocalConfig):
            ...
            logger.warning(
                f"Unsupported config type: {type(self.config)}. Defaulting to Gemini initialisation."
            )
            self._init_gemini_macla()
    ...
    def _provide_feedback(
        self, prev_state_str: str, cur_state_str: str, cur_obs_image: Any = None
    ) -> dict | None:
        """
        ...
        strong_success, is_fatal_game_over = self._detect_success(
            self._last_execution_result, prev_state_str, cur_state_str
        )
        ...
        logger.debug(
            f"Providing feedback to MACLA: strong_success={strong_success}, is_fatal_game_over={is_fatal_game_over}, shaped_reward={shaped_reward:.3f} Image: {cur_obs_image}"
        )
    ...
    def _execute_task(
        ...
        memory_stats = {}
        if (
            self._macla_step_count > 0
            and self._macla_step_count % self._learning_step_interval == 0
        ):
            ...
            logger.info(
                f"MACLA Stats & Optimisation (Step {self._macla_step_count}): {memory_stats}"
            )
    ...
    def _build_reasoning_with_macla_metadata(
        self, execution_result: dict, action: str = None, goal: str = None
    ) -> str:
        """
    ...
    def _log_stats(self, update_info: dict | None, memory_stats: dict):
        ...
        if update_info:
            ...
            if wandb.run and self._macla_step_count % 10 == 0:
                log_data = {
                    ...
                    "learning_rate": self._update_counters["procedure_learned"]
                    / max(self._macla_step_count, 1),
                }
    ...
    def _build_procedure_table(
        self, procedure_list: list[dict], table_type: str = "procedure"
    ) -> tuple[list, list]:
        """
    ...
    def _record_episode_end(self, episode: int, score: float):
        ...
        if self._macla_agent and hasattr(self._macla_agent, "memory_system"):
            ...
            if hasattr(mem, "last_iter_score"):
                mem.last_iter_score = float(score)

        # Track episode length
        self._episode_lengths.append(self._steps_in_current_episode)
        avg_steps = (
            sum(self._episode_lengths) / len(self._episode_lengths) if self._episode_lengths else 0
        )
        ...
        try:
            ...
            if wandb.run:
                ...
                if procedures_data.get("procedures"):
                    ...
                    logger.info(
                        f"DEBUG: creating learned_procedures table with {len(data)} rows. Columns: {columns}"
                    )
                    if data:
                        logger.info(
                            f"DEBUG: First row image check - Pre: {data[0][9] is not None}, Post: {data[0][10] is not None}"
                        )
                ...
                if procedures_data.get("meta_procedures"):
                    ...
                    episode_stats["learned_meta_procedures"] = wandb.Table(
    ...
    def _maybe_render_stuck_detector(
        ...
        signal = self._loop_detector.observe(state=state, score=score, action_class=action_class)
    ...
    def get_action(self, obs: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        if len(result_tuple) == 8:
            ...
            action, reasoning, output_text, memory_stats, prompt, game_phase, update_type = (
                result_tuple
            )
    ...
    def load_state(self, state: dict[str, Any]) -> None:
        ...
        if "macla_memory" in state and self._macla_agent:
            ...
            threshold = getattr(adapter, "PROC_CACHE_MIN_ITER_SCORE", None)
            if threshold is not None and hasattr(mem, "prune_low_score_iter"):
