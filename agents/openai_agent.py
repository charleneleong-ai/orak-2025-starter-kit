...
class BaseOpenAIAgent(BaseOrakAgent):
    """
    Base agent for OpenAI models using LangChain.
    Handles initialization of the ChatOpenAI client.
    """
    ...
    _llm: ChatOpenAI | None = PrivateAttr(default=None)

    def __init__(self, config: OpenAIConfig = None, wandb_config: WandbConfig = None, **kwargs):
        # Load configurations
        config = config or OpenAIConfig()
        wandb_config = wandb_config or WandbConfig()

        super().__init__(config=config, wandb_config=wandb_config, **kwargs)
    ...
    def _initialize_llm(self):
        ...
        logger.info(
            f"Initialized OpenAI agent with model: {self.config.model}, reasoning: {is_reasoning_model}"
        )
