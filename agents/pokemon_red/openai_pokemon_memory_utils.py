        ...
        if i + 1 < len(sorted_notables_coords):
            ...
            if coord1[1] == coord2[1] and coord2[0] == coord1[0] + 1:
                ...
                if "WarpPoint" in val1 and "WarpPoint" in val2:
                    ...
                    merged_notables.append(
                        f"({coord1[0]:2}-{coord2[0]:2}, {coord1[1]:2}) {final_text}"
                    )
                    skip_next = True
                    merged = True
...
def replace_map_on_screen_with_full_map(
    state_text: str, map_current: list, warp_annotations: dict = None
) -> str:
    """
    ...
    if (
        not map_current
        or not isinstance(map_current, list)
        or not (all(isinstance(row, list) for row in map_current) if map_current else True)
    ):
        return state_text
    ...
    # Parsed but not currently consumed downstream — kept as documentation
    # of the format the upstream observation-emitter promises.
    _player_x, _player_y = -1, -1
    player_pos_match = re.search(r"Your position \(x, y\): \((\d+), (\d+)\)", processed_state_text)
    if player_pos_match:
        _player_x = int(player_pos_match.group(1))
        _player_y = int(player_pos_match.group(2))
    ...
    if not map_current:
        ...
        if num_cols == 0:
            ...
            if num_cols > 0:
                if num_cols >= 100:
                    col_headers_digits_only_list.append(
                        "".join([str(i // 100 % 10) if i >= 100 else " " for i in range(num_cols)])
                    )
                if num_cols >= 10:
                    col_headers_digits_only_list.append(
                        "".join([str(i // 10 % 10) if i >= 10 else " " for i in range(num_cols)])
                    )
            ...
            for y_coord in range(num_rows):
                ...
                for x_coord in range(num_cols):
                    ...
                    if val_at_cell and isinstance(val_at_cell, str):
                        if len(val_at_cell) == 1:
                            ...
                            if val_at_cell == "WarpPoint" or "WARP" in val_at_cell.upper():
                                ...
                                if not is_known_warp:
                                    notable_objects[(x_coord, y_coord)] = (
                                        f"{val_at_cell} (Unexplored)"
                                    )
                                else:
                        ...
                        notable_objects[(x_coord, y_coord)] = (
                            f"E: Invalid_Data_Type({type(val_at_cell).__name__})"
                        )
            ...
            if warp_annotations:
                for coord, note_text in warp_annotations.items():
                    if coord not in notable_objects:
                        ...
                        notable_objects[coord] = (
                            f"WarpPoint -> {note_text}"
                            if "->" not in note_text
                            and ("Map" not in note_text and "SPRITE" not in note_text)
                            else note_text
                        )
                    else:
...
def replace_filtered_screen_text(state_text: str, dialog_buffer: list) -> str:
    ...
    new_section = "[Interacted Dialog Buffer]\n" + "\n".join(dialog_buffer) + "\n\n"
...
def get_warp_annotations(
    warp_memory: dict, map_memory: dict, current_map_name: str, map_current: list
) -> dict:
    ...
    for pos, dest_map in raw_warps.items():
        ...
        if map_current:
            ...
            is_current_warp = (
                "WARP" in curr_cell.upper()
                or "DOOR" in curr_cell.upper()
                or curr_cell == "WarpPoint"
            )
            ...
            if not is_current_warp:
                ...
                if 0 <= y - 1 < len(map_current) and 0 <= x < len(map_current[0]):
                    ...
                    if (
                        "WARP" in up_cell.upper()
                        or "DOOR" in up_cell.upper()
                        or up_cell == "WarpPoint"
                    ):
                        display_pos = (x, y - 1)
                        snapped = True
...
def detect_npc_interaction(parsed_state: dict, map_current: list) -> str:
    ...
    if screen_text and pos and facing and map_current:
        ...
        if 0 <= ty < len(map_current) and 0 <= tx < len(map_current[0]):
            ...
            if isinstance(target_obj, str) and (
                target_obj.startswith("SPRITE_") or target_obj.startswith("SIGN")
            ):
                return target_obj
    return None
