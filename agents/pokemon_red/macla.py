...
from agents.pokemon_red.base import USER_PROMPT_TEMPLATE, PokemonRedAgent
...
class PokemonRedMaclaAgent(BaseMaclaAgent, PokemonRedAgent):
    ...
    def _get_action(
        ...
        current_goal = (
            self._current_goal
            if self._current_goal
            else self._get_task_description({"task_description": task_description})
        )
        reasoning = self._build_reasoning_with_macla_metadata(
            execution_result, action=action, goal=current_goal
        )
    ...
    def _detect_success(
        ...
        try:
            ...
            for indicator in progress_indicators:
                if (
                    indicator.lower() in cur_state_str.lower()
                    and indicator.lower() not in prev_state_str.lower()
                ):
                    strong_success = True
                    break
    ...
    def _extract_context(self, observation: str | list) -> dict[str, Any]:
        ...
        try:
            ...
            if "[Current Party]" in observation:
                party_section = observation.split("[Current Party]")[1].split(
                    "[" if "[" in observation.split("[Current Party]")[1] else "No more Pokemons"
                )[0]
                party_count = len(
                    [l for l in party_section.split("\n") if l.strip() and "Name:" in l]
                )
    ...
    def extract_preconditions(self, context_key: str, observation: str) -> list[str]:
        ...
        try:
            ...
            if (
                "[Full Map]" in observation
                and "?" in observation.split("[Full Map]")[1].split("\n[")[0]
            ):
