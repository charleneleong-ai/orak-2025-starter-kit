...
from .actions import ActionDescriptions
...
def sc2_run_game(transaction, lock, isReadyForNextStep, game_end_event, done_event,
    ...
    map = map_name
    replay_path = f'{log_path}/replay.SC2Replay'
...
def sc2_run_multi_game(transaction1, transaction2, lock1, lock2,
    ...
    # 运行游戏并获取结果
    result = run_game(maps.get(map),
                      [Bot(Race.Protoss, Protoss_Bot(transaction1, lock1, isReadyForNextStep1)),
                       Bot(Race.Protoss, Protoss_Bot(transaction2, lock2, isReadyForNextStep2)),],
                      realtime=False,
                      save_replay_as=replay_path)
...
class Protoss_Bot(BotAI):
    ...
    def get_information(self):
        ...
        information = {
            ...
            "in_progress": self._get_in_progress_information(),
    ...
    def _get_building_information(self):
        ...
        # }
        return {
            'nexus_count': max(0, self.base_count - self.already_pending(UnitTypeId.NEXUS)),
            'pylon_count': max(0, self.pylon_count - self.already_pending(UnitTypeId.PYLON)),
            'gas_buildings_count': max(0, self.gas_buildings_count - self.already_pending(UnitTypeId.ASSIMILATOR)),
            'gateway_count': max(0, self.gateway_count - self.already_pending(UnitTypeId.GATEWAY)),
            'forge_count': max(0, self.forge_count - self.already_pending(UnitTypeId.FORGE)),
            'photon_cannon_count': max(0, self.photon_cannon_count - self.already_pending(UnitTypeId.PHOTONCANNON)),
            'shield_battery_count': max(0, self.shield_battery_count - self.already_pending(UnitTypeId.SHIELDBATTERY)),
            'warp_gate_count': max(0, self.warp_gate_count - self.already_pending(UnitTypeId.WARPGATE)),
            'cybernetics_core_count': max(0, self.cybernetics_core_count - self.already_pending(UnitTypeId.CYBERNETICSCORE)),
            'twilight_council_count': max(0, self.twilight_council_count - self.already_pending(UnitTypeId.TWILIGHTCOUNCIL)),
            'robotics_facility_count': max(0, self.robotics_facility_count - self.already_pending(UnitTypeId.STARGATE)),
            'statgate_count': max(0, self.stargate_count - self.already_pending(UnitTypeId.STARGATE)),
            'templar_archives_count': max(0, self.templar_archives_count - self.already_pending(UnitTypeId.TEMPLARARCHIVE)),
            'dark_shrine_count': max(0, self.dark_shrine_count - self.already_pending(UnitTypeId.DARKSHRINE)),
            'robotics_bay_count': max(0, self.robotics_bay_count - self.already_pending(UnitTypeId.ROBOTICSBAY)),
            'fleet_beacon_count': max(0, self.fleet_beacon_count - self.already_pending(UnitTypeId.FLEETBEACON)),
        }
    ...
    def _get_in_progress_information(self):
        """Retrieve in_progress-related information."""
        return {
            # 建筑相关
            "building_constructing": {
                'constructing_nexus_count': self.already_pending(UnitTypeId.NEXUS),
                'constructing_pylon_count': self.already_pending(UnitTypeId.PYLON),
                'constructing_gas_buildings_count': self.already_pending(UnitTypeId.ASSIMILATOR),
                'constructing_gateway_count': self.already_pending(UnitTypeId.GATEWAY),
                'constructing_forge_count': self.already_pending(UnitTypeId.FORGE),
                'constructing_photon_cannon_count': self.already_pending(UnitTypeId.PHOTONCANNON),
                'constructing_shield_battery_count': self.already_pending(UnitTypeId.SHIELDBATTERY),
                'constructing_warp_gate_count': self.already_pending(UnitTypeId.WARPGATE),
                'constructing_cybernetics_core_count': self.already_pending(UnitTypeId.CYBERNETICSCORE),
                'constructing_twilight_council_count': self.already_pending(UnitTypeId.TWILIGHTCOUNCIL),
                'constructing_robotics_facility_count': self.already_pending(UnitTypeId.ROBOTICSFACILITY),
                'constructing_stargate_count': self.already_pending(UnitTypeId.STARGATE),
                'constructing_templar_archives_count': self.already_pending(UnitTypeId.TEMPLARARCHIVE),
                'constructing_dark_shrine_count': self.already_pending(UnitTypeId.DARKSHRINE),
                'constructing_robotics_bay_count': self.already_pending(UnitTypeId.ROBOTICSBAY),
                'constructing_fleet_beacon_count': self.already_pending(UnitTypeId.FLEETBEACON),
            },
            # 单位相关
            "unit_producing": {
                'producing_probe_count': self.already_pending(UnitTypeId.PROBE),
                'producing_zealot_count': self.already_pending(UnitTypeId.ZEALOT),
                'producing_stalker_count': self.already_pending(UnitTypeId.STALKER),
                'producing_sentry_count': self.already_pending(UnitTypeId.SENTRY),
                'producing_adept_count': self.already_pending(UnitTypeId.ADEPT),
                'producing_high_templar_count': self.already_pending(UnitTypeId.HIGHTEMPLAR),
                'producing_dark_templar_count': self.already_pending(UnitTypeId.DARKTEMPLAR),
                'producing_immortal_count': self.already_pending(UnitTypeId.IMMORTAL),
                'producing_colossus_count': self.already_pending(UnitTypeId.COLOSSUS),
                'producing_disruptor_count': self.already_pending(UnitTypeId.DISRUPTOR),
                'producing_archon_count': self.already_pending(UnitTypeId.ARCHON),
                'producing_observer_count': self.already_pending(UnitTypeId.OBSERVER),
                'producing_warp_prism_count': self.already_pending(UnitTypeId.WARPPRISM),
                'producing_phoenix_count': self.already_pending(UnitTypeId.PHOENIX),
                'producing_voidray_count': self.already_pending(UnitTypeId.VOIDRAY),
                'producing_oracle_count': self.already_pending(UnitTypeId.ORACLE),
                'producing_carrier_count': self.already_pending(UnitTypeId.CARRIER),
                'producing_tempest_count': self.already_pending(UnitTypeId.TEMPEST),
                'producing_mothership_count': self.already_pending(UnitTypeId.MOTHERSHIP),
            }
        }
    ...
    async def on_step(self, iteration: int):
        ...
        with self.lock:
            ...
            # print(self.temp_failure_list)
            if len(self.temp_failure_list) == 0:
                if self.action_dict[action] != 'EMPTY ACTION':
                    self.transaction['action_executed'] = copy.deepcopy(self.action_dict[action])
                    await self.chat_send(self.action_dict[action])
            #
