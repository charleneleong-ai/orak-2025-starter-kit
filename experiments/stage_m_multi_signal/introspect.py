"""Stage K vs L vs M introspection: compare cumulative-memory mechanics.
...
helps" from "negative transfer":
  ...
  - viridian_steps: how many steps on any Viridian map (the M5 ceiling)
...
Usage:
    uv run python experiments/stage_m_multi_signal/introspect.py \
        --stage-l /tmp/orak-stage-l-map-aware/pokemon_red \
        --stage-m /tmp/orak-stage-m-multi-signal/pokemon_red
"""
...
def main(
    stage_l: Path = typer.Option(
        ...
        "--stage-l",
        help="Stage L (baseline) GAME_DATA_DIR/pokemon_red root",
    ),
    stage_m: Path = typer.Option(
        Path("/tmp/orak-stage-m-multi-signal/pokemon_red"),
        "--stage-m",
        help="Stage M (treatment) GAME_DATA_DIR/pokemon_red root",
    ),
    glob_l: str = typer.Option("stage_l_map_aware_iter*", "--glob-l"),
    glob_m: str = typer.Option("stage_m_multi_signal_iter*", "--glob-m"),
):
    """Print a side-by-side table of Stage L vs Stage M iter metrics."""
    for tag, base, glob in (("L", stage_l, glob_l), ("M", stage_m, glob_m)):
        print(f"\n══════ Stage {tag} ({base}) ══════")
        rows = []
