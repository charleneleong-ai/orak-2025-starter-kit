#!/usr/bin/env bash
# Stage P — every-step map-graph observation augmentation.
...
#       — 26 procs (6.5x M's 4), Route1 dwell up to 110 in iter 4,
#         but 0 Viridian steps across all 5 iters.
#
# Stage P bars:
#   Minimum: any Viridian step OR final_map ends in Viridian for any iter.
#   Lift:    any iter past 57.14%.
#
# Smoke confirmed (50 steps):
#   graph_hint fired 17+ times across RedsHouse1f/RedsHouse2f.
#   Wiring verified — hint reaches the planner observation every step.
set -uo pipefail
...
N=5
TAG="stage_p_map_graph"
RESULTS_DIR="experiments/stage_p_map_graph"
mkdir -p "$RESULTS_DIR"

export GAME_DATA_DIR="/tmp/orak-stage-p"
...
for iter in $(seq 1 $N); do
    ...
    cmd=(uv run python run.py
        ...
        --run-id "$run_id"
        -d "Stage P map-graph: iter $iter (inherit from ${prev_run_id:-NONE})"
    )
...
row = {
    ...
    "steps": 300,
    "status": "KEEP",
    "description": f"Stage P: every-step map-graph observation augmentation; {len(scores)}x pokemon cumulative memory",
    "notes": f"n={len(scores)}: mean={mean:.2f}% std={std:.2f}pp scores=[{fmt}] learning_delta={delta:+.2f}pp",
    "tags": ["stage_p_map_graph", "cumulative_memory", "pokemon_red"],
