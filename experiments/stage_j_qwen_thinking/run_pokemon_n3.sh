#!/usr/bin/env bash
# Stage J — Qwen3-30B-A3B-Thinking-2507-AWQ-4bit n=3 on pokemon Stage D.
#
...
#                  → 57.14% × n=3, σ=0
#   Stage J model: cyankiwi/Qwen3-30B-A3B-Thinking-2507-AWQ-4bit
...
#                     reasoning style. Stage K cumulative memory becomes the
#                     next move.
...
for iter in $(seq 1 $n); do
    run_id="pr_stage_j_qwen3_thinking_pokemon_iter${iter}_$(date -u +%Y%m%dT%H%M%SZ)"
    started=$(date +%s)
    echo "================================================================"
    echo "[$(date -u +%H:%M:%SZ)] STAGE J n=3 iter $iter/$n"
    ...
    if ! uv run python run.py \
        ...
        --run-id "$run_id" \
        -d "Stage J iter $iter: Qwen3-Thinking 30B-A3B-Int4 — extended-reasoning budget at decision time"; then
...
echo
echo "================================================================"
echo "[$(date -u +%H:%M:%SZ)] STAGE J n=3 SUMMARY"
echo "================================================================"
python3 <<PYEOF
...
row = {
    ...
    "steps": 300,
    "status": "KEEP",
    "description": "Stage J: Qwen3-Thinking 30B-A3B-Int4 extended-reasoning budget at decision time",
    "notes": f"n=3: mean={mean:.2f}% std={std:.2f}pp scores={fmt}. delta_vs_stage_h={mean-57.14:+.2f}pp",
    "tags": ["qwen", "qwen3-thinking-30b-a3b", "stage_j_thinking_mode", "stage_j_qwen3_thinking_pokemon_n3"],
    "wandb_url": "",
