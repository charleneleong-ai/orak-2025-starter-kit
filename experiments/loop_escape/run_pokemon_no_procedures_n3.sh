#!/usr/bin/env bash
# Stage B' — raw-LLM-with-reflection baseline (no procedure cache), n=3.
#
# Tests the competing hypothesis to PR #68: maybe the 57.14% pokemon ceiling
# is upstream of the procedure layer (LLM/planner combo capped at milestone 4
# regardless of architecture). Disable ONLY the procedure cache; keep
# self-reflection + planner + vmem ON. Run 3 times for variance.
#
# Read of outcomes:
#   Stage B' mean ≈ 57.14% with low std  → LLM/planner ceiling is real; PR #68
#                                          procedure-escape fixes will also plateau
#   Stage B' mean > 57.14% OR high std   → procedure cache was suppressing
#                                          exploration; PR #68 (or just dropping
#                                          procedures) is the right play
#
# Toggled via LocalConfig.use_procedure_layer = false. All reflection
# enhancements (use_self_reflection, use_subtask_planning, use_vector_memory)
# stay ON via per-game adapter recommendations.
set -uo pipefail
...
for iter in $(seq 1 $n); do
    run_id="pr_procesc_stage_b_prime_iter${iter}_$(date -u +%Y%m%dT%H%M%SZ)"
    started=$(date +%s)
    echo "================================================================"
    echo "[$(date -u +%H:%M:%SZ)] STAGE B' n=3 iter $iter/$n"
    echo "  config: gemma_26b_no_procedures (procedure layer OFF; reflection/planner/vmem ON)"
    echo "  run_id=$run_id"
    echo "================================================================"
    ...
    if ! uv run python run.py \
        ...
        --run-id "$run_id" \
        -d "Stage B' iter $iter: pokemon Stage D - procedure cache, +reflection — does raw-LLM ceiling at 57.14%?"; then
        echo "[FAIL] iter $iter exited non-zero — recording 0"
        scores+=("0.0")
        continue
    fi
...
row = {
    ...
    "variant": "stage_b_prime_no_procedures_n3",
    "game": "pokemon_red",
    ...
    "steps": 300,
    "status": "KEEP",
    "description": "Stage B' n=3: Stage D minus procedure cache (reflection/planner/vmem all ON). Tests LLM/planner ceiling vs procedure-layer hypothesis.",
    "notes": f"n=3: mean={mean:.2f}% std={std:.2f}pp scores={fmt}",
    "tags": ["loop_escape", "stage_b_prime_no_procedures_n3"],
    "wandb_url": "",
