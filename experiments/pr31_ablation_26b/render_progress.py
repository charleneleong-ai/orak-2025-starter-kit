...
def render() -> Path:
    ...
    for y, v in zip(ys, ORDER):
        ...
        if r is None:
            ...
            ax.text(2, y, "pending", va="center", color="#9CA3AF", fontsize=10, style="italic")
            pending.append(v)
        else:
            score_pct = r["evaluation_score"]
            game = r["game_score"]
            steps = r["steps"]
            ax.barh(y, score_pct, color=COLOURS[v], edgecolor="#1F2937", linewidth=1, alpha=0.92)
            label = f"  {game:.0f}/7  ·  {score_pct:.2f}%  ·  {steps} steps"
            text_x = score_pct + 1
            ax.text(text_x, y, label, va="center", fontsize=11, color="#1F2937", fontweight="bold")
            completed.append(v)

    # Prior plateau reference line
    ax.axvline(PRIOR_PLATEAU, color="#DC2626", linestyle=":", linewidth=1.5, alpha=0.8)
    ax.text(
        PRIOR_PLATEAU + 0.6,
        -0.55,
