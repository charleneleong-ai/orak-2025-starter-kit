#!/usr/bin/env bash
# Stage C++ retest — same MMR+decay vmem config as stage_c_26b_mmr_decay
# but at 600 steps instead of 300. Tests whether the agent can clear
# milestones beyond #2 once the retrieval reinforcement loop is broken.
#
# Compares against:
#   stage_c_26b           = 0/7  (0.00%)  300 steps  (vanilla vmem-only collapse)
#   stage_c_26b_mmr_decay = 2/7  (28.57%) 300 steps  (loop broken, parity with Stage A)
#   stage_d_plus_26b      = 5/7  (71.43%) 600 steps  (vmem ON + planner ON + grace=10)
#
# Launch as detached daemon (PPID=1):
#   setsid nohup ./experiments/pr31_ablation_26b/run_stage_c_plus_mmr_decay.sh \
#     </dev/null >>logs/stage_c_plus_mmr_decay_$(date -u +%Y%m%dT%H%M%SZ).log 2>&1 & disown
set -uo pipefail
...
stage="stage_c_plus_26b_mmr_decay"
desc="Stage C++ retest 26B: vmem ON + MMR + decay (alpha=0.5, lambda=0.5), planner OFF, 600 steps — extend the 28.57% MMR+decay result and test for progress past milestone 2"
...
echo "  vmem=true (MMR+decay)  planner=false  max_steps=600"
echo "  run_id=$run_id"
echo "================================================================"
