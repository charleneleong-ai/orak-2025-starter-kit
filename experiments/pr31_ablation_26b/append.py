...
def append(
    ...
    print(
        f"Appended {row['experiment']}: {variant} score={final}/7 ({eval_pct:.2f}%) status={status}"
    )
    return row


def render() -> Path:
    # Import render_progress as a sibling module so this works whether
    # invoked from the repo root or from anywhere with PYTHONPATH unset.
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from render_progress import render as _render_custom  # type: ignore
