...
ORDER = [
    ...
    "stage_a_obs_fix",
    "stage_c_26b",
    "stage_c_26b_mmr_decay",
    "stage_c_plus_26b_mmr_decay",
    "stage_c_plus_26b_mmr_decay_t03",
    "stage_b_26b",
    "stage_d_26b",
    "stage_d_26b_self_reflect",
    "stage_d_plus_26b",
]
LABELS = {
    ...
    "stage_a_obs_fix": (
        "Stage A + obs",
        "+ obs preprocessor (#61)",
        "vmem OFF · planner OFF · expanded obs",
    ),
    "stage_c_26b": ("Stage C", "+ vmem", "vmem ON · planner OFF"),
    "stage_c_26b_mmr_decay": (
        "Stage C′",
        "+ MMR + decay",
        "vmem ON (MMR λ=0.5 · decay α=0.5) · planner OFF",
    ),
    "stage_c_plus_26b_mmr_decay": (
        "Stage C′++",
        "+ MMR + decay (600st)",
        "vmem ON (MMR λ=0.5 · decay α=0.5) · planner OFF · 600st",
    ),
    "stage_c_plus_26b_mmr_decay_t03": (
        "Stage C′++ T=0.3",
        "+ MMR + decay + low temp",
        "vmem ON (MMR λ=0.5 · decay α=0.5) · planner OFF · 600st · T=0.3",
    ),
    ...
    "stage_d_26b_self_reflect": (
        "Stage D + reflect",
        "+ self-reflection (#62)",
        "vmem ON · planner ON · reflect_every=10",
    ),
    "stage_d_plus_26b": (
        "Stage D++",
        "both (600st + grace)",
        "vmem ON · planner ON · 600st · grace=10",
    ),
}
...
def find_run_dir(variant: str) -> Path | None:
    ...
    pat = (
        "pr31_rerun_pokemon_2026*"
        if variant == "stage_d_26b"
        else f"pr31_rerun_pokemon_{variant}_*"
    )
...
def overview_table(rows_by_v: dict, stats_by_v: dict) -> str:
    out = [
        "| Stage | Switches | Score | % | Steps | Stuck-fire | Status |",
        "|---|---|---|---|---|---|---|",
    ]
    for v in ORDER:
...
def verdict(rows_by_v: dict) -> str:
    ...
    a = score("stage_a_26b")
    c = score("stage_c_26b")
    cmd = score("stage_c_26b_mmr_decay")
    cppmd = score("stage_c_plus_26b_mmr_decay")
    aobs = score("stage_a_obs_fix")
    b = score("stage_b_26b")
    d = score("stage_d_26b")
    dpp = score("stage_d_plus_26b")
    ...
    if d is not None and a is not None:
        lines.append(
            f"> - Stage D vs Stage A: **{d - a:+.2f} pp** — full stack adds this much over model-only."
        )
    if c is not None and a is not None:
        lines.append(f"> - vmem-only lift (C − A): **{c - a:+.2f} pp**")
    if cmd is not None and c is not None:
        lines.append(
            f"> - **MMR + decay rescue** (C′ − C): **{cmd - c:+.2f} pp** — retrieval reinforcement loop broken"
        )
    if cmd is not None and a is not None:
        delta = cmd - a
        verdict_word = "parity with" if abs(delta) < 1 else ("above" if delta > 0 else "below")
        lines.append(
            f"> - vmem-with-MMR-decay vs Stage A (C′ − A): **{delta:+.2f} pp** — {verdict_word} model-only baseline"
        )
    if cppmd is not None and cmd is not None:
        lines.append(f"> - C′++ vs C′ (600st extension): **{cppmd - cmd:+.2f} pp**")
    if aobs is not None and a is not None:
        delta = aobs - a
        lines.append(
            f"> - **Obs preprocessor (#61) on Stage A**: {aobs:.2f}% vs Stage A's {a:.2f}% "
            f"({delta:+.2f} pp) — preprocessor active but agent never moved south to expose RedsHouse1f y≥6; "
            f"necessary-not-sufficient without an exploration nudge."
        )
    ...
    if c is not None and b is not None:
        if b > c + 5:
            lines.append(
                f"> - **Planner is the load-bearing lever** ({b:.2f}% vs vmem's {c:.2f}%)."
            )
        elif c > b + 5:
            lines.append(
                f"> - **Vmem is the load-bearing lever** ({c:.2f}% vs planner's {b:.2f}%)."
            )
        else:
            lines.append(
                f"> - **Planner ≈ vmem** in isolation ({b:.2f}% vs {c:.2f}%) — the levers may compose multiplicatively."
            )
...
def build_comment(rows: list[dict]) -> str:
    ...
    body += [
        "</details>",
        "",
        "_Plot regenerated by `experiments/pr31_ablation_26b/render_progress.py` after each completed run; rolling comment maintained by `post_retro.py`._",
    ]
    return "\n".join(body)
...
def upsert_comment(body_md: str) -> int:
        ...
        try:
            subprocess.check_output(
                [
                    "gh",
                    "api",
                    f"repos/{GH_REPO}/issues/comments/{cid}",
                    "-X",
                    "PATCH",
                    "-F",
                    f"body=@{body_path}",
    ...
    out = subprocess.check_output(
        [
            "gh",
            "api",
            f"repos/{GH_REPO}/issues/{PR_NUM}/comments",
            "-X",
            "POST",
            "-F",
            f"body=@{body_path}",
...
def update_pr_body(rows: list[dict]) -> None:
    ...
    subprocess.run(
        ["gh", "api", f"repos/{GH_REPO}/pulls/{PR_NUM}", "-X", "PATCH", "-F", f"body=@{body_path}"],
        check=True,
