...
#
# Stage S Step 1 n=5 sweep (FLAT, 57.14% × 5) falsified the cache-
# inheritance hypothesis. Trajectory introspection of the orchestrator
# log showed every iter reached Route1 then bounced back to Pallet, none
# ever entered ViridianCity. Diagnosis: EnterViridian's completion
# predicate is score-based (obs["score"] >= 5) and only fires once the
# env detects the agent has already entered Viridian — chicken-and-egg
# with no spatial pull for the planner once Route1's nav popped.
#
# v1 fix (agents/pokemon_red/game_adapter.py:initial_subgoal_stack):
# insert NavigateToMap(ViridianCity) below the Route1 nav subgoal. After
# Route1 pops on entry, the planner sees "Walk until current map is
# ViridianCity" -> emits move_to(x, 0) in Route1 -> engine warps ->
# ViridianCity nav pops -> EnterViridian (score) exposed -> env ticks.
#
# Stage S v1 bars:
#   Minimum: no iter < 50%.
#   Lift:    mean > 57.14% (Viridian reached at least once in n=5).
#   Stretch: >=2 iters past 71.43% (sustained M5+).
set -uo pipefail
...
N=5
TAG="stage_s_v1_viridian_bridge"
RESULTS_DIR="experiments/stage_s_v1_viridian_bridge"
MAX_STEPS=600
mkdir -p "$RESULTS_DIR"

export GAME_DATA_DIR="/workspace/orak-stage-s-v1"
...
# Pre-flight: Stage S v1 — ViridianCity nav subgoal bridge installed
...
grep -q '_navigate_to_map("ViridianCity")' agents/pokemon_red/game_adapter.py \
    || { echo "FATAL: Stage S v1 ViridianCity nav bridge missing"; exit 1; }
echo "[preflight] Stage S v1 NavigateToMap(ViridianCity) bridge present"
...
for iter in $(seq 1 $N); do
    ...
    cmd=(uv run python run.py
        ...
        --run-id "$run_id"
        -d "Stage S v1 — ViridianCity nav bridge (every iter fresh): iter $iter"
    )
...
log_experiment(
    ...
    status="IN_PROGRESS",
    description=f"Stage S v1 ViridianCity-bridge iter {os.environ['ITER']}/{os.environ['N_ITERS']} (fresh)",
...
results_path = log_experiment(
    ...
    status="KEEP",
    description=(
        f"Stage S v1 — ViridianCity nav subgoal bridge: v5 lever stack + "
        f"NavigateToMap(ViridianCity) between Route1 and EnterViridian; "
        f"budget{steps}, every iter fresh"
    ),
