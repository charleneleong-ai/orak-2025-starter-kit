#!/usr/bin/env bash
# Stage S v2 — chain past Viridian: M6 ViridianMart + M7 OaksLab bridges.
#
# v1 broke the Stage D M4 wall (mean 74.29%, +17.15pp over baseline) but
# 4/5 iters still stuck at M5 — they entered Viridian then wandered with
# no spatial directive for M6 (parcel pickup at the Mart) or M7 (return
# to Oak's Lab for delivery). Only iter4 punched M6 at 85.71%, proving
# the score gates above M5 are reachable but unreliable without their
# own spatial scaffolding.
#
# v2 fix: extend _POKEMON_MILESTONE_LIBRARY so M6 declares
#   requires_location="ViridianMart" and M7 declares
#   requires_location="OaksLab".
# The framework (MilestoneSpec + build_score_milestone_stack, lifted
# from v1's manual insert in PR #103) auto-inserts the corresponding
# NavigateToMap bridge above each milestone. Pop order top→bottom is now
#   Route1 → ViridianCity → M5 → ViridianMart → M6 → OaksLab → M7.
#
# Stage S v2 bars:
#   Minimum: no iter < 71.43% (don't regress v1's M5-secured floor).
#   Lift:    mean > 74.29% (improve v1's mean — 6/7 more often than not).
#   Stretch: >=2 iters at 100% (full Oak's parcel quest end-to-end).
set -uo pipefail
...
N=5
TAG="stage_s_v2_post_viridian_chain"
RESULTS_DIR="experiments/stage_s_v2_post_viridian_chain"
MAX_STEPS=600
mkdir -p "$RESULTS_DIR"

export GAME_DATA_DIR="/workspace/orak-stage-s-v2"
...
# Pre-flight: Stage S v2 — M5/M6/M7 each declare requires_location in the library
...
grep -q 'requires_location="ViridianCity"' agents/pokemon_red/game_adapter.py \
    || { echo "FATAL: M5 requires_location=ViridianCity missing (v1 carry-over)"; exit 1; }
grep -q 'requires_location="ViridianMart"' agents/pokemon_red/game_adapter.py \
    || { echo "FATAL: Stage S v2 M6 requires_location=ViridianMart missing"; exit 1; }
grep -q 'requires_location="OaksLab"' agents/pokemon_red/game_adapter.py \
    || { echo "FATAL: Stage S v2 M7 requires_location=OaksLab missing"; exit 1; }
echo "[preflight] Stage S v2 M5/M6/M7 requires_location declarations present"

# Framework lift (PR #103): MilestoneSpec must exist + builder accepts nav_factory
grep -q "class MilestoneSpec" agents/macla/macla_lib.py \
    || { echo "FATAL: MilestoneSpec dataclass missing — PR #103 not landed"; exit 1; }
grep -q "nav_factory" agents/macla/macla_lib.py \
    || { echo "FATAL: build_score_milestone_stack(nav_factory=...) missing"; exit 1; }
echo "[preflight] PR #103 framework lift (MilestoneSpec + nav_factory) wired"
...
for iter in $(seq 1 $N); do
    ...
    cmd=(uv run python run.py
        ...
        --run-id "$run_id"
        -d "Stage S v2 — post-Viridian chain (every iter fresh): iter $iter"
    )
...
log_experiment(
    ...
    status="IN_PROGRESS",
    description=f"Stage S v2 post-Viridian-chain iter {os.environ['ITER']}/{os.environ['N_ITERS']} (fresh)",
...
results_path = log_experiment(
    ...
    status="KEEP",
    description=(
        f"Stage S v2 — post-Viridian chain: v5 lever stack + MilestoneSpec."
        f"requires_location bridges (ViridianCity / ViridianMart / OaksLab) "
        f"declared on M5/M6/M7 via framework auto-insert; budget{steps}, "
        f"every iter fresh"
    ),
