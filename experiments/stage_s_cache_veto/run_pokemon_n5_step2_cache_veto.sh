#!/usr/bin/env bash
# Stage S Step 2 — cache veto under escape-valve fire (inheritance ON).
#
# Runs only if Step 1 (no-inherit baseline) lifted the ceiling — that
# confirms cache inheritance was the wall and gives Step 2 a target to
# match. If Step 1 was flat at ~57%, the wall isn't inheritance and
# this sweep is skipped.
#
# Step 2 hypothesis: when the subgoal escape valve fires (stagnation
# >= 30 steps), the cache-veto window opens for CACHE_VETO_K_STEPS=30
# steps and fresh planning wins arbitration. With inheritance ON, the
# cache still provides cumulative speedup for the early game, but the
# veto prevents iter1's mid-trajectory procs from poisoning later
# iters' boundary-crossing path. Should land between Step 1's mean
# (upper bound) and v5's mean (42.86% — broken inheritance) — ideally
# matching Step 1 without sacrificing cumulative learning.
#
# Implementation: identical to the v5 sweep launcher except the cache
# veto code (commit d070f07) is now LIVE — when stagnation fires,
# set_cache_veto(30) is called from unified.py, and the selector
# short-circuits cached-proc selection.
#
# Stage S Step 2 bars:
#   Minimum: no iter < 50% (don't regress v3's floor).
#   Lift:    mean > 57.14% AND within 7pp of Step 1's mean.
#   Stretch: >=2 iters past 71.43% (sustained M5 lift, not iter1-only).
set -uo pipefail
...
N=5
TAG="stage_s_cache_veto_step2"
RESULTS_DIR="experiments/stage_s_cache_veto_step2"
MAX_STEPS=600
mkdir -p "$RESULTS_DIR"

export GAME_DATA_DIR="/workspace/orak-stage-s-step2"
...
echo "[preflight] Stage S cache veto wired — ACTIVE with inheritance ON"
...
for iter in $(seq 1 $N); do
    ...
    cmd=(uv run python run.py
        ...
        --run-id "$run_id"
        -d "Stage S Step 2 — cache veto under escape-valve fire (inherit ON): iter $iter (inherit from ${prev_run_id:-NONE})"
    )
...
log_experiment(
    ...
    status="IN_PROGRESS",
    description=f"Stage S Step 2 cache-veto iter {os.environ['ITER']}/{os.environ['N_ITERS']} (inherit_from={os.environ['PREV_RUN_ID'] or 'NONE'})",
...
PYEOF
    # Stage S Step 2: inheritance ON — propagate prev_run_id for cache
    # passthrough. The cache veto window (CACHE_VETO_K_STEPS=30 when
    # escape valve fires) protects against the v4/v5 PalletTown-loop
    # symptom while preserving cumulative early-game speedup.
    prev_run_id="$run_id"
done
...
results_path = log_experiment(
    ...
    status="KEEP",
    description=(
        f"Stage S Step 2 — cache veto under escape-valve fire: v5 lever stack, "
        f"budget{steps}, inheritance ON; veto opens for K=30 steps when "
        f"stagnation >= 30 (commit d070f07)"
    ),
