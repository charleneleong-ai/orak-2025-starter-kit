#!/usr/bin/env bash
# Stage R v5 — single-lever follow-up to v4: bump perf-prune threshold.
#
# v4 n=5 introspection (docs/experiments/stage_r_subgoals/v4_n5_introspection.md)
# showed:
#   - iter1 (fresh) cleared the M5 ceiling at 71.43% — first ever above
#     the v1/v3 57.14% wall. v4(0) graph_hint + v4(3) 600-step budget +
#     v4(6) M5-M7 subgoal stack all firing correctly.
#   - iter2-5 inherited iter1's procs but only iter4 even reached Route1
#     (step 350, way late). Cumulative memory was speeding up the
#     cutscene early-game (iter4: M1@11/M2@22/M3@77/M4@86 - faster than
#     iter1) but failed to surface iter1's late boundary-crossing path.
#   - Root cause: PROC_CACHE_MIN_ITER_SCORE = 4.0 sat exactly at the
#     score iter2/iter4 hit (4.0/7). Both never left PalletTown but
#     were retained as "good" procs -> iter3/iter5 thrashed in Pallet.
#
# v5 single-character fix: PROC_CACHE_MIN_ITER_SCORE 4.0 -> 5.0 in
# agents/pokemon_red/game_adapter.py. M5 (5.0 = EnterViridian) is the
# smallest score that proves the agent actually crossed Pallet->Route1
# ->Viridian, so it's the right keep gate. All other v4 levers stay.
#
# Expected outcome: iter2/iter4 (if they still hit 4.0) get pruned ->
# iter1's Viridian-reaching procs flow through the chain -> later iters
# can build on iter1's spatial knowledge instead of iter2/iter4's
# Pallet-only loop.
#
# Stage R v5 bars:
#   Minimum: no iter < 50% (don't regress v3's floor).
#   Lift:    mean > 57.14% (break the ceiling on average, not just iter1).
#   Stretch: >=2 iters past 71.43% (sustain the v4 iter1 peak).
set -uo pipefail
...
N=5
TAG="stage_r_subgoals_v5"
RESULTS_DIR="experiments/stage_r_subgoals_v5"
MAX_STEPS=600
mkdir -p "$RESULTS_DIR"

export GAME_DATA_DIR="/tmp/orak-stage-r-subgoals-v5"
...
# Pre-flight: Stage R v5 = v4 levers + PROC_CACHE_MIN_ITER_SCORE bumped to 5.0
grep -q "^PROC_CACHE_MIN_ITER_SCORE = 5.0" agents/pokemon_red/game_adapter.py \
    || { echo "FATAL: v5 PROC_CACHE_MIN_ITER_SCORE not 5.0"; exit 1; }
echo "[preflight] v5 PROC_CACHE_MIN_ITER_SCORE=5.0 (M5 / EnterViridian gate)"
...
for iter in $(seq 1 $N); do
    ...
    cmd=(uv run python run.py
        ...
        --run-id "$run_id"
        -d "Stage R v5 (v4 levers + perf-prune threshold 4.0->5.0): iter $iter (inherit from ${prev_run_id:-NONE})"
    )
...
log_experiment(
    ...
    status="IN_PROGRESS",
    description=f"Stage R v5 iter {os.environ['ITER']}/{os.environ['N_ITERS']} (inherit_from={os.environ['PREV_RUN_ID'] or 'NONE'})",
...
results_path = log_experiment(
    ...
    status="KEEP",
    description=(
        f"Stage R v5: v4 levers + perf-prune threshold 5.0 (was 4.0); budget{steps}; "
