#!/usr/bin/env bash
# Stage R v3 — soft subgoal preference + stagnation escape valve (F1+F2).
# n=5 cumulative-memory sweep, fresh start (separate GAME_DATA_DIR).
#
# Diagnosis (Stage R v2 introspection, docs/experiments/stage_r_subgoals/
# v2_n5_introspection.md): v2 scored 28.57 / 28.57 / 57.14 — iter 1+2
# stuck in PalletTown after starter pickup because the v2 "HARD
# CONSTRAINT" planner phrasing locked the planner when move_to(12, 0)
# reliably stalled at (12, 5) (couldn't cross the Pallet→Route1 edge).
#
# Stage R v3 interventions (planner-side only, no tool changes):
#   F1) Soften the planner prompt — "Currently pursuing ... Prefer
#       Subtasks that advance this unless the recent history shows
#       it's blocked." replaces v2's HARD CONSTRAINT / MUST language.
#       System prompt heuristic 0 is now "strong preference, not lockout."
#   F2) Stagnation escape valve — unified.py drops the active_subgoal
#       block from the planner prompt entirely once the top of the
#       subgoal stack has held for SUBGOAL_STAGNATION_THRESHOLD=30
#       steps. Block re-engages on the next stack mutation (pop/push).
#
# Stage R v3 bars:
#   Minimum: no iter score < 50% (v2's 28.57% iter 1+2 sag does not recur).
#   Lift:    >=2 iters past 57.14% OR mean > 57.14% (anything v2 iter 3
#            never reached).
set -uo pipefail
...
N=5
TAG="stage_r_subgoals_v3"
RESULTS_DIR="experiments/stage_r_subgoals_v3"
mkdir -p "$RESULTS_DIR"

export GAME_DATA_DIR="/tmp/orak-stage-r-subgoals-v3"
...
grep -q "record_subgoal_step" agents/macla/macla_lib.py \
...
for iter in $(seq 1 $N); do
    ...
    cmd=(uv run python run.py
        ...
        --run-id "$run_id"
        -d "Stage R v3 (soft + escape valve): iter $iter (inherit from ${prev_run_id:-NONE})"
    )
...
row = {
    ...
    "steps": 300,
    "status": "KEEP",
    "description": f"Stage R v3: soft subgoal + stagnation escape valve; {len(scores)}x pokemon cumulative memory",
    "notes": f"n={len(scores)}: mean={mean:.2f}% std={std:.2f}pp scores=[{fmt}] learning_delta={delta:+.2f}pp",
    "tags": ["stage_r_subgoals_v3", "cumulative_memory", "pokemon_red"],
