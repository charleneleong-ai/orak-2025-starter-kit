"""Append a 2048 rerun row to results.jsonl.

Mirrors experiments/pr31_ablation_26b/append.py but for 2048 — picks up
final_score + step count from evaluation_summary.json and writes a row in
the autoresearch shape (max_eval = log2(max_tile) units, post-#46).
"""
...
SWEEP_DIR = REPO / "experiments/pr31_2048_rerun/gemma_26b"
...
def append(
    ...
    row = {
        ...
        "notes": (
            f"max_eval={final:.2f} (log2 max_tile units, post-#46), {n_eps} ep, {steps} steps"
        ),
        "tags": ["pr31_2048_rerun", variant],
        "wandb_url": "",
